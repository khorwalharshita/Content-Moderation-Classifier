...
class Dataset1(Dataset):
    def __init__(self, data_path, tokenizer, max_source_length, max_target_length, target_lang):
        fp = open(data_path, 'r')
    ...
    def __getitem__(self, idx):
        input_text = ' '.join(self.df[idx]['input_text'])
        target_text = self.df[idx]['target_text']
...
class DataModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        self.train = Dataset1(self.hparams.train_path, self.tokenizer, self.hparams.max_source_length, self.hparams.max_target_length, self.hparams.target_lang)
        self.val = Dataset1(self.hparams.val_path, self.tokenizer, self.hparams.max_source_length, self.hparams.max_target_length, self.hparams.target_lang)
        self.test = Dataset1(self.hparams.test_path, self.tokenizer, self.hparams.max_source_length, self.hparams.max_target_length, self.hparams.target_lang)
